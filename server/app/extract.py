...
SYSTEM_MOD = (
    ...
    "from $X to $Y' phrasing is present) the previous cumulative. "
    "`cumulative_obligated` is the total for the WHOLE CONTRACT after this action, "
    "and many SF-30s never state one. Fill it ONLY from a figure the document "
    "presents as a running total — the '$Y' of 'increased from $X to $Y', a "
    "parenthetical '(cumulative obligated $Y)' printed beside this action's amount, "
    "or a phrase like 'total obligated amount of this contract to date'. Transcribe "
    "it digit for digit and re-read it before answering: it is a larger number than "
    "the action amount sitting next to it, and one wrong leading digit multiplies "
    "the contract's recorded funding. A line reading "
    "'Obligated this action $X' is `amount_obligated` and NOTHING else: copying it "
    "into `cumulative_obligated` states that this one mod is the entire funding "
    "history of the contract, which erases every obligation that came before it. "
    "When no running total is printed, leave `cumulative_obligated` null — null is "
    "read correctly downstream, a wrong figure is not. Classify the "
